...
const AddShowtime = () => {
  ...
  return (
    ...
    <div className="relative mt-4 grid  gap-6">
      ...
      <div>
        <label className="block text-sm font-medium text-black" htmlFor="sdate"> Show Date </label>
        <input required  onChange={event => setShowDate(event.target.value)} name="sdate" type="date" className="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 px-4 py-2 text-black focus:border-blue-500 focus:outline-none focus:ring sm:text-sm" />
      </div>

      <div>
        <label className="block text-sm font-medium text-black" htmlFor="price"> Show Time </label>
        <input
          type="text"
          step="1"
